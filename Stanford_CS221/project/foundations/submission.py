...
# BEGIN_YOUR_CODE (our solution is 20 lines of code, but don't worry if you deviate from this)
    ...
    # Create a dictionary to hold pairs of adjacent words
    adjacency = collections.defaultdict(set)

    # Populate the adjacency dictionary with pairs of adjacent words
    for i in range(n - 1):
        adjacency[words[i]].add(words[i + 1])
    
    # Function to recursively build similar sentences
    def build_sentences(current_sentence: List[str], length: int):
        if len(current_sentence) == length:
            similar_sentences.add(" ".join(current_sentence))
            return
        ...
        if last_word is not None:
            for next_word in adjacency[last_word]:
                build_sentences(current_sentence + [next_word], length)
    
    similar_sentences = set()
    
    for start_word in words:
        build_sentences([start_word], n)
