...
def create_chain_csp(n: int) -> CSP:
    ...
    def are_different(x, y):
        return x != y
    for i in range(0,n):
        csp.add_variable(variables[i],domains)
    for i in range(0,n):
        if i != n-1:
            csp.add_binary_factor(variables[i],variables[i+1],are_different)
    # END_YOUR_CODE
    return csp
...
def create_nqueens_csp(n: int = 8) -> CSP:
    ...
    # Step 1: Define the variables (each variable represents a queen)
    variables = [f'Q{i}' for i in range(n)]  # Variables for each queen
    domain = list(range(n))  # Each queen can be placed in any of the n columns

    # Step 2: Add each variable with its domain to the CSP
    for var in variables:
        csp.add_variable(var, domain)

    # Step 3: Add binary factors (constraints) between each pair of queens
    for i in range(n):
        for j in range(i + 1, n):
            # Use a lambda function to define the binary constraints
...
class BacktrackingSearch:
    ...
    def get_unassigned_variable(self, assignment: Dict):
        ...
        if not self.mcv:
            ...
            # Heuristic: most constrained variable (MCV)
            # Initialize minimum count and variable
            min_count = float('inf')
            selected_var = None

            for var in self.csp.variables:
                if var not in assignment:
                    # Get the remaining domain values
                    remaining_values = self.domains[var]
                    count_remaining = sum(1 for value in remaining_values if self.satisfies_constraints(assignment, var, value))
                    
                    # Update if we found a variable with fewer remaining values
                    if count_remaining < min_count:
                        min_count = count_remaining
                        selected_var = var
                    elif count_remaining == min_count:
                        # Tie-breaking: choose the variable with the lowest index
                        selected_var = selected_var if selected_var is not None and self.csp.variables.index(selected_var) < self.csp.variables.index(var) else var
...
class SchedulingCSPConstructor:
    ...
    def add_quarter_constraints(self, csp: CSP) -> None:
        ...
        for request in self.profile.requests:
            # Only proceed if specific quarters are specified
            if request.quarters:
                # Iterate through all quarters in the profile
                for quarter in self.profile.quarters:
                    # If this quarter is not specified in request.quarters, restrict it
                    if quarter not in request.quarters:
                        # Ensure the course cannot be taken in this quarter
    ...
    def add_unit_constraints(self, csp: CSP) -> None:
        ...
        for quarter in self.profile.quarters:
            # Step 1: Initialize a list to hold the unit variables for this quarter.
            unit_variables = []

            for request in self.profile.requests:
                for cid in request.cids:
                    # Step 2: Get the course info from the bulletin.
                    course_info = self.bulletin.courses[cid]
                    
                    # Step 3: Create a variable for units taken for the course in this quarter.
                    units_var = (cid, quarter)
                    # Create domain including 0 and the allowed range of units
                    domain = [0] + list(range(course_info.minUnits, course_info.maxUnits + 1))
                    csp.add_variable(units_var, domain)
                    

                    # Collect the units variable for the quarter
                    unit_variables.append(units_var)
                    csp.add_binary_factor((request,quarter),units_var ,lambda request_cid,course_unit,cid=cid:course_unit>0 if (request_cid==cid) else course_unit==0)

            # Step 6: Create a sum variable for the total units in this quarter.
            total_units_var = create_sum_variable(csp, f'{quarter}_units', unit_variables, self.profile.maxUnits)

            # Step 7: Add constraints to ensure total units are within min/max range.
            csp.add_unary_factor(total_units_var, lambda total: self.profile.minUnits <= total <= self.profile.maxUnits)
        # END_YOUR_CODE
