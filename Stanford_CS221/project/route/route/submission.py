...
def getStanfordWaypointsShortestPathProblem() -> WaypointsShortestPathProblem:
    ...
    startLocation = str(1707961578)
    waypointTags = ["landmark=evgr_a", "crossing=unmarked", "kerb=lowered"]
    endTag = "kerb=lowered"
    # END_YOUR_CODE
...
def aStarReduction(problem: SearchProblem, heuristic: Heuristic) -> SearchProblem:
    class NewSearchProblem(SearchProblem):
        ...
        def successorsAndCosts(self, state: State) -> List[Tuple[str, State, float]]:
            ...
            new_list=  problem.successorsAndCosts(state)
            for member in new_list:
                member = list(member)
                member[2] += heuristic.evaluate(member[1]) - heuristic.evaluate(state) # Use A* search
                member = tuple(member)
...
class StraightLineHeuristic(Heuristic):
    ...
    def __init__(self, endTag: str, cityMap: CityMap):
        ...
        # Precompute
        # BEGIN_YOUR_CODE (our solution is 5 lines of code, but don't worry if you deviate from this)
        self.endlocation = [location for location in self.cityMap.geoLocations.keys() if endTag in self.cityMap.tags[location]]
        # END_YOUR_CODE

    def evaluate(self, state: State) -> float:
        # BEGIN_YOUR_CODE (our solution is 6 lines of code, but don't worry if you deviate from this)
        return max([computeDistance(self.cityMap.geoLocations[state.location], self.cityMap.geoLocations[endLocation]) for endLocation in self.endlocation])
        # END_YOUR_CODE
