...
class ExactInferenceWithSensorDeception(ExactInference):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        # Convert the agent's row and column indices to locations

        # Calculate the adjusted observed distance based on skewness
        adjusted_dist = (1 / (1 + self.skewness**2)) * observedDist + \
                        ((2 / (1 + self.skewness**2)) ** 0.5)

        # Update probabilities based on the transformed observed distance
        for row in range(self.numRows):
            for col in range(self.numCols):
                x = util.colToX(col)
                y = util.rowToY(row)
                prior = self.belief.getProb(row,col)
                likelihood= util.pdf(math.sqrt((agentX - x) ** 2 + (agentY - y) ** 2), Const.SONAR_STD, adjusted_dist)
                prob = prior * likelihood # formula
                self.belief.setProb(row,col,prob)
