...
def naiveSoftmaxLossAndGradient(
    ...
    # Calculate softmax
    softmax_probs = softmax(np.dot(outsideVectors, centerWordVec))

    # Calculate loss
    loss = -np.log(softmax_probs[outsideWordIdx])

    # Compute gradients
    softmax_probs[outsideWordIdx] -= 1
    gradCenterVec = np.dot(outsideVectors.T, softmax_probs)
    gradOutsideVecs = np.outer(softmax_probs, centerWordVec)
